...
df_cls = pd.read_csv('cls_data.csv',parse_dates=['Date'], index_col=['Date'], dayfirst=True).fillna(method='ffill') #readcsv with FFIL
df_mkt_cap = pd.read_csv('mc_data.csv',parse_dates=['Date'], index_col=['Date'], dayfirst=True).fillna(method="ffill")

df_fm = pd.read_csv('fm_21.csv', parse_dates=['Date'], index_col=['Date'], dayfirst=True)
...
beta.to_csv('beta.csv', encoding='utf-8')
